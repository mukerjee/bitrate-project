...
one proxy to each server. Once you've started playing both videos, run the
topo1 events file (instructions in \S\ref{sec:netsim}).

\textbf{TODO: details about what plots they should make as they vary $\alpha$}
...
\begin{center}
	\texttt{$<$time$>$ $<$request-tput$>$ $<$avg-tput$>$ $<$bitrate$>$ $<$request-ip$>$ $<$chunkname$>$}
\end{center}

\begin{description}
	\item[\texttt{time}] The current time in seconds since the epoch.
	\item[\texttt{request-tput}] The throughput you measured for the current
	chunk in Kbps.
	...
	Kbps.
	\item[\texttt{request-ip}] The client IP address from which the proxy
...
\begin{center}
	\texttt{./proxy $<$log$>$ $<$listen-port$>$ $<$fake-ip$>$ $<$dns-ip$>$ $<$dns-port$>$ [$<$www-ip$>$]}
\end{center}
